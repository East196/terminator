 ...
 * @return Specification
 */
	public static <T> Specification<T> byStringFieldsLike(final String seach, 
			final Class<T> klass) {
		return new Specification<T>() {
			@Override
			public Predicate toPredicate(Root<T> root, CriteriaQuery<?> query, CriteriaBuilder builder) {
				List<String> fieldNames=Reflections.fieldNameByKlass(klass, String.class);
				if (Collections3.isNotEmpty(fieldNames)) {
					List<Predicate> predicates = Lists.newArrayList();
				...
				if(fieldNames.contains("enable")) builder.or(builder.equal(root.get("enable"), true));
				return builder.conjunction();
			}
		};
	}
